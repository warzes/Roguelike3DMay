...
namespace Utility
{
	namespace // helpers
	{
		...
		std::vector<gl4::Texture> LoadImages(const fastgltf::Asset& asset)
		{
			...
			{
				Extent2D dims = { static_cast<uint32_t>(image.width), static_cast<uint32_t>(image.height) };
	...
	std::vector<Material> LoadMaterials(const fastgltf::Asset& model, std::span<gl4::Texture> images)
	{
		...
		{
			assert(loaderMaterial.pbrData.has_value());
	...
	{
		assert(scene.textures.size() == scene.samplers.size());
		const auto baseMaterialIndex = static_cast<uint32_t>(scene.materials.size());
